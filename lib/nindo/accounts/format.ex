defmodule Nindo.Format do
  ...
  def profile_picture(username) when is_binary(username),
    do: profile_picture(Accounts.get_by_username(username))
  ...
  def description(username) when is_binary(username),
    do: description(Accounts.get_by_username(username))
  ...
  def display_name(username) when is_binary(username),
    do: display_name(Accounts.get_by_username(username))
  ...
  def default_profile_picture(username),
    do: "https://avatars.dicebear.com/api/identicon/#{username}.svg?scale=80&b=%23E5E7EB"
